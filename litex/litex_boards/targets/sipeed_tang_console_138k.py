...
from litex_boards.platforms import sipeed_tang_console_138k
...
class BaseSoC(SoCCore):
    def __init__(self, toolchain="gowin", sys_clk_freq=50e6,
        ...
        with_buttons        = True,
        **kwargs):
        platform = sipeed_tang_console_138k.Platform(toolchain=toolchain)

        assert not with_sdram or (sdram_model in ["sipeed", "mister"])

        if with_sdram:
            platform.add_extension({
                "sipeed": sipeed_tang_console_138k.sipeedSDRAM(),
                "mister": sipeed_tang_console_138k.misterSDRAM}[sdram_model]
            )
        ...
        # SoCCore ----------------------------------------------------------------------------------
        SoCCore.__init__(self, platform, sys_clk_freq, ident="LiteX SoC on Tang Console 138K", **kwargs)
        if cpu_clk_freq:
...
def main():
    ...
    parser = LiteXArgumentParser(platform=sipeed_tang_console_138k.Platform, description="LiteX SoC on Tang Console 138K.")
